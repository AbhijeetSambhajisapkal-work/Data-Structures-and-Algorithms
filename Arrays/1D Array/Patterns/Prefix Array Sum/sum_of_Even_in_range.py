...
def sum_of_even_in_range(A):
    ...
    for i in range(1,len(A)):
        if(i%2==0):
            ...
            pfA[i] = pfA[i-1];
    print(f"prefix subarray even sum is :{pfA}");   
    return pfA;


def get_sum_even_elements_in_range(A,B):
    queries = len(B);
    pfx = sum_of_even_in_range(A);
    diff = [];
    for i in range(queries):
        ...
        if(l==0):
            ...
            suma = pfx[r]-pfx[l-1];
        diff.append(suma);
    print(f"prefix subarray even sum in {queries} : {B} given ranges is:{diff}");   
    return diff;
...
if __name__ == "__main__":
    A = [2, 8, 3, 9, 15]
    B = [[1, 4],[0, 2],[2, 3]]; 
    sum_of_even_in_range(A);
    get_sum_even_elements_in_range(A,B);
